...
function _Modelo6() {
  let modelo7maisModelo6 = document.getElementById('mod7-mod6');
  if (modelo7maisModelo6) {
    ...
    function cliqueOn(elClicado) {
      $(elClicado).css('background', '#007BFF');
    ...
    function checkCertas() {
      for (var i = 0; i < numQuestoes; i++) {
        if (cliques[i] % 2 !== 0) {
          if ($(`#checkbox${i + 1}`).hasClass('certaModelo6')) {
  ...
  if (modelo7maisModelo6) {
    ...
    function checkCertas() {
      for (questao of questoes) {
        if ($(questao).hasClass('certaModelo7') && $(questao).hasClass('selecionada')) {
    ...
    function alerta() {
      ...
      $('#alerta-modelo7').css('marginLeft', '190px');
